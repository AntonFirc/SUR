...
def process_dataset(dataset_path, output_path, thread_pool=4):
    ...
    with ThreadPool(thread_pool) as pool:
        list(
            tqdm(
                pool.imap(
                    process_speaker,
...
def process_speaker(speaker_path):
    ...
    speaker_idx = int(str(speaker_path).split('/').pop())

    for speaker_file in speaker_path.iterdir():
        if str(speaker_file).endswith('.wav'):
            audio, sample_rate = read_wave(str(speaker_file))
            frames = frame_generator(90, audio, sample_rate)
            frames = list(frames)
            vad = webrtcvad.Vad(3)
            max_len = 0
            segments = vad_collector(sample_rate, 90, 300, vad, frames)
            for i, segment in enumerate(segments):
                if len(segment) > max_len:
                    path = outpath.joinpath(str(speaker_idx)).joinpath(speaker_file.basename())
